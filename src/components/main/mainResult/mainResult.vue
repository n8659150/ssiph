			...
			<div class="list-main">
				<div class="result-wrap">
					...
					<div class="list-search-input-wrapper">
						<input type="text" class="list-search-inputarea" placeholder="在结果中筛选" v-model="filterWords" value="">
					</div>
					...
					<hiPagination v-show="showAllResults & total > 7" :total="total" :current-page='current' @pagechange="pagechange"></hiPagination>
				</div>
			</div>
		</div>
	</div>
...
export default {
		...
		async queryById(id) {
			...
			if (!response) {
				...
				if (performanceCurve.resultType == "0") {
					...
					console.log("性能曲线无数据");
					localStorage.setItem("performanceCurveData", JSON.stringify(self.performanceCurveDataInit));
	...
	computed: {
		recommendedResults() {
			let keywords = this.filterWords.toLowerCase();
			let results = this.$store.state.recommendedResults;
        	return (results || []).filter(result => result.productName.toLowerCase().indexOf(keywords) !== -1)
		},
		searchResults() {
			let keywords = this.filterWords.toLowerCase();
		...
		// searchIn() {
        // if (this.search !== '') {
        //   return this.$store.state.recommendedResults.filter(function (item) {
        //     return item.productName.indexOf(this.search) > -1
        //   })
        //  }
		// }
	},

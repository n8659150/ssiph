			...
			<div class="result-wrap">
				<ul class="list-tab-nav">
					<li ng-class="tab-active">
						<a href="javascript:void(0)">推荐结果</a>
					</li>
					<li ng-class="{'tab-active':showAll.active}">
						<a href="javascript:void(0)">所有结果</a>
					</li>
				</ul>
					...
					<div class="list-table-body">
						<table class="tablebox">
							<tbody class="list-table-tbody">
								...
								<tr class="list-table-tr" v-for="(result,key,index) in results" :key="key">
									<!-- 名称 -->
									<td class="list-table-td col-wd-150">
										<div>
											<!-- <span><a ui-sref="frame.detail({id:result.id})">{{result.productName}}</a></span> -->
											<span><a href="javascript:void(0)">{{result.productName}}</a></span>
										</div>
									</td>
									<!-- 领域 -->
...
export default {
  ...
  data () {
    return {
      results: [{
        capMeasCond:"ASHRAE/T",
        cop:3.15,
        lengmei:0.01,
        paiqiliang:0.73,
        lengliang:3000,
        dianyuan:"220V-240V~/50HZ/1φ",
        productType:"定速",
        territory:"轻商",
        productName:"TH470MV-C9EU"
      },
      {
        capMeasCond:"ASHRAE/T",
        cop:3.15,
